...
my $FORMATDB;
my $ABRES_DB;
my ($Help, $AB_indput, $InFile,$threshold, $dir);
my $IFormat = "fasta";
my $OFormat = "ST";
...
$contigtable .= "-----------\n";
my $Plasmidtable ="\nPlasmid\t\tNote\n";
$Plasmidtable .= "_________________________________________________________________________________\n";
my $alignment = "\n\nAlignments as text\n_________________________________________________________________________________\n";
my @headarray = ("PLASMID      ", "IDENTITY     ", "QUERY/HSP     ", "CONTIG  ", "POSITION         ", "ACC NO.         ");
...
foreach my $element(@Antimicrobial){
  ...
  foreach my $contig (keys %HoA_sort){
    ...
    #for (my $x = 0; $x <= $#sorted; $x++){
      #for (my $y = 0; $y <= $#{$sorted[$x]}; $y++) {
        #print "At X=$x and Y=$y is ", $sorted[$x][$y], "\n";
      #}
      #print "\n";
    #}
    ...
    until ($next > $#sorted ) { # så længe $first <= 4 og $next <= 5
      ...
      if ($sorted[$first][$end] <= $sorted[$next][$start]) { # If first end is before next start
        ...
        if ($next == $#sorted){
          push (@best_hit, "$sorted[$first][$start]_$sorted[$first][$end]_$sorted[$first][$contig]");
          #print "1>2: $next\tSavec: $sorted[$first][$start]_$sorted[$first][$end]_$sorted[$first][$contig]\n";
          $next++;
        } else {
        ...
        if ($next == $#sorted){
          push (@best_hit, "$sorted[$next][$start]_$sorted[$next][$end]_$sorted[$next][$contig]");
          #print "1<=2: $next\tSaved:$sorted[$next][$start]_$sorted[$next][$end]_$sorted[$next][$contig]\n";
          $next++;
        } else {
          #print "1<=2: first: $first, next: $next\n";
          $first = $next;
        ...
        if ($sorted[$first][$bits] > $sorted[$next][$bits]) { # First bits > next bits => first better
          if ($next == $#sorted){
            push (@best_hit, "$sorted[$first][$start]_$sorted[$first][$end]_$sorted[$first][$contig]");
            #print "1>2: $next\tsave: $sorted[$first][$start]_$sorted[$first][$end]_$sorted[$first][$contig]\n";
            $next++;
          } else {
            #print "1>2: first: $first, next: $next\n";
            $next++;
          ...
          if ($next == $#sorted){
            push (@best_hit, "$sorted[$next][$start]_$sorted[$next][$end]_$sorted[$next][$contig]");
            #print "1<=2: $next\tsave: $sorted[$next][$start]_$sorted[$next][$end]_$sorted[$next][$contig]\n";
            $next++;
          } else {
            #print "1<=2: first: $first, next: $next\n";
            $first = $next;
        ...
        push (@best_hit, "$sorted[$first][$start]_$sorted[$first][$end]_$sorted[$first][$contig]");
        #print "Save: $sorted[$first][$start]_$sorted[$first][$end]_$sorted[$first][$contig]";
        $next++;
      }
    }
  }
   ...
   #Finding genes with a %ID >= threshold, savning results for output-print in html
    if ($HSP_LENGTH{$data} >= (3/5)*$QUERY_LENGTH{$data} and $PERC_IDENT{$data} >= $threshold) {
      ...
      if ($HSP_LENGTH{$data} == $QUERY_LENGTH{$data} and $PERC_IDENT{$data} == 100.00){ #If 100% length and %id = 100
        ...
        push(@{$GENE_RESULTS_HASH{$gene}}, $data); # e.g. tet(A)_2_AC12345
        push(@{$GENE_RESULTS_HASH{$gene}}, $note); #Phenotype
...
=pod  print "GENE\tIDENTITY\tAllele Length/HSP\tContig name\tPosition in contig\tAccession nr\n";
  foreach my $key (sort keys %GENE_RESULTS_HASH) {
    #print "\t".$key."\n";
    my $array = $GENE_RESULTS_HASH{$key};
    print  @$array[9]."\t".@$array[1]."%\t".@$array[2]."/".@$array[3]."\t".@$array[4]."\t".@$array[5]."\t".@$array[8]."\n";
  }
=cut print "\n";
...
foreach my $key (sort keys %FINAL_RESULTS) {
  ...
  $txtresults .= $header."\n" ;
  $txtresults .="*********************************************************************************\n";
  $txtresults .="PLASMID      IDENTITY     QUERY/HSP     CONTIG  POSITION         ACC NO.         \n";
  $txtresults .="=================================================================================\n";
  ...
  $txtresults .= @$array[0].$spacestr.@$array[1].$spacestr1.@$array[2]."/".@$array[3].$spacestr2.$contigcount.$spacestr3.@$array[5].$spacestr4.@$array[6].$spacestr5."\n";
...
foreach my $key (sort keys %GENE_RESULTS_HASH2) {
  ...
  for (my $i=0; $i < scalar(@$hitArray); $i++){
	 ...
	 $alignment .= "Plasmid seq:         ".$tmpQuerySingleLine."\n";
...
if(-e "error.log"){}else{system("rm -r error.log");}
if(-e "formatdb.log"){}else{system("rm -r formatdb.log");}
system("rm -r  $tmp_dir/");
